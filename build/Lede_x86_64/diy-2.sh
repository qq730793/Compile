...
sed -i "/uci commit network/i\uci set network.lan.dns='223.5.5.5 119.29.29.29'" $ZZZ   # DNS(多个DNS要用空格分开)
...
sed -i "/uci commit system/i\uci set system.@system[0].hostname='OpenWrt-weige'" $ZZZ       # 修改主机名称为OpenWrt-123

sed -i "s/OpenWrt /weige build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ        # 增加自己个性名称281677160
